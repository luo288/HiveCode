...
def _session_path(hivecode_dir: Path) -> Path:
    return hivecode_dir / SESSION_FILENAME
...
def save_worktree_session(
    ...
    path = _session_path(hivecode_dir)
    if session is None:
...
def load_worktree_session(hivecode_dir: Path) -> WorktreeSession | None:
    path = _session_path(hivecode_dir)
    if not path.exists():
        return None
    try:
