...
def load_instructions(project_root: str) -> str:
    ...
    发现顺序（低优先级在前，高优先级在后）：
    1. 用户全局：~/.hivecode/HIVECODE.md, ~/.hivecode/AGENTS.md
    2. 项目目录链：从 git root 到 workDir，每个目录的 HIVECODE.md 和 AGENTS.md
    3. workDir/.hivecode/INSTRUCTIONS.md（遗留格式）
    4. workDir/HIVECODE.local.md（本地覆盖）
    """
    ...
    # 1. 用户全局
    _add(home / ".hivecode" / "HIVECODE.md")
    _add(home / ".hivecode" / "AGENTS.md")
    ...
    # 3. 遗留格式
    _add(root / ".hivecode" / "INSTRUCTIONS.md")

    # 4. 本地覆盖
    _add(root / "HIVECODE.local.md")
