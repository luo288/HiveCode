...
from hivecode.config import ConfigError, load_config
from hivecode.hooks import HookConfigError, HookEngine, load_hooks
from hivecode.permissions import PermissionMode


def main() -> None:
    # 先确保 .hivecode/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".hivecode").mkdir(parents=True, exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".hivecode/debug.log",
        filemode="w",
    )

    parser = argparse.ArgumentParser(prog="hivecode", description="HiveCode AI coding assistant")
    parser.add_argument(
        "--mode",
    ...
    if args.remote:
        from hivecode.remote import RemoteServer
    ...
    from hivecode.app import HiveCodeApp
    from hivecode.driver import NoAltScreenDriver
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str, output_format: str = "text") -> None:
    ...
    from hivecode.client import create_client, resolve_context_window
    from hivecode.conversation import ConversationManager
    from hivecode.memory.instructions import load_instructions
