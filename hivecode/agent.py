...
from hivecode.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from hivecode.conversation import ThinkingBlock as ConvThinkingBlock
...
from hivecode.hooks import HookContext, HookEngine, ToolRejectedError
from hivecode.hooks.engine import HookNotification
from hivecode.prompts import build_environment_context, build_plan_mode_reminder, build_system_prompt
from hivecode.tools import ToolRegistry
from hivecode.tools.base import (
    MAX_OUTPUT_CHARS,
    StreamEnd,
    StreamEvent,
    TextDelta,
...
class Agent:
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".hivecode" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".hivecode" / "plans"
    ...
    async def run(self, conversation: ConversationManager) -> AsyncIterator[AgentEvent]:
        ...
        while True:
            ...
            # system reminders 已注入，一次生成包含所有内容的 API 视图
            ...
            # 用 Layer 1 裁剪后的消息估算 token，判断更精确
            compact_result = await auto_compact(
                ...
                budget_messages=api_conv.history,
            )
            if isinstance(compact_result, CompactEvent):
                ...
                # 压缩后 conv 已变，重建 API 视图
                api_conv, _ = apply_tool_result_budget(
                    conversation, self.session_dir, self.replacement_state
                )
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from hivecode.permissions.rules import Rule, extract_content
    ...
    async def run_to_completion(
        ...
        for iteration in range(1, self.max_iterations + 1):
            ...
            pre_compact_conv, pre_compact_records = apply_tool_result_budget(
                conversation, self.session_dir, self.replacement_state
            )
            ...
            compact_result = await auto_compact(
                ...
                tool_schemas=self.registry.get_all_schemas(self.protocol),
                transcript_path=self._transcript_path,
                budget_messages=pre_compact_conv.history,
            )
            if isinstance(compact_result, CompactEvent):
                conversation.inject_environment(env_context)
