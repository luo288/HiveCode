"""从 .hivecode/commands/ 目录加载自定义 Markdown 命令（对齐 Go 版 loader.go）。
...
from hivecode.commands.registry import Command, CommandContext, CommandType
...
def load_user_commands(work_dir: str) -> list[Command]:
    ...
    搜索路径（后者覆盖前者）：
      1. ~/.hivecode/commands/
      2. <work_dir>/.hivecode/commands/
    """
    dirs: list[str] = []
    home = Path.home()
    dirs.append(str(home / ".hivecode" / "commands"))
    dirs.append(str(Path(work_dir) / ".hivecode" / "commands"))
