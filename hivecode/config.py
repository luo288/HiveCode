...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".hivecode" / "config.yaml",
        cwd / ".hivecode" / "config.yaml",
        cwd / ".hivecode" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .hivecode/config.yaml "
            "in project or ~/.hivecode/config.yaml"
        )
    return merged
