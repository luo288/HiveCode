...
from hivecode.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
)
from hivecode.commands.completion import CompletionPopup
from hivecode.commands.handlers import register_all_commands
from hivecode.config import MCPServerConfig, ProviderConfig
from hivecode.hooks import HookContext, HookEngine, load_hooks
from hivecode.conversation import ConversationManager, Message
from hivecode.mcp import ConnectResult, MCPManager
from hivecode.memory import (
    MemoryManager,
    Session,
    SessionManager,
...
from hivecode.permissions import (
...
from hivecode.agents.task_manager import TaskManager
from hivecode.agents.trace import TraceManager
from hivecode.agents.notification import inject_task_notifications
from hivecode.commands.handlers.tasks import create_tasks_command
from hivecode.skills.executor import SkillExecutor
from hivecode.skills.loader import SkillLoader
from hivecode.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from hivecode.cache import FileCache
from hivecode.tools import ToolRegistry, create_default_registry
from hivecode.tools.agent_tool import AgentTool
from hivecode.tools.ask_user import AskUserEvent, AskUserTool
from hivecode.tools.impl.tool_search import ToolSearchTool
from hivecode.tools.load_skill import LoadSkill
from hivecode.worktree.cleanup import start_stale_cleanup_task
from hivecode.worktree.manager import WorktreeManager
from hivecode.commands.handlers.worktree import create_worktree_command
from hivecode.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".hivecode", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".hivecode" / "history"
...
THINKING_VERBS = [
    ...
    "Marinating", "Meandering", "Metamorphosing", "Hiving", "Moonwalking",
...
class HiveCodeApp(App):
    ...
    def __init__(
        ...
        from hivecode.config import SandboxAppConfig
    ...
    def on_mount(self) -> None:
        self.register_theme(_HIVECODE_THEME)
        self.theme = "hivecode"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".hivecode" / "permissions.yaml",
                project_rules_path=Path(work_dir) / ".hivecode" / "permissions.yaml",
                local_rules_path=Path(work_dir) / ".hivecode" / "permissions.local.yaml",
            ),
        ...
        if self._sandbox_cfg.enabled:
            from hivecode.sandbox import SandboxConfig, create_sandbox
            os_sandbox = create_sandbox()
            if os_sandbox and os_sandbox.available():
                sandbox_config = SandboxConfig(
                    allow_write=[work_dir, "/tmp"],
                    deny_write=[
                        f"{work_dir}/.hivecode/config.yaml",
                        f"{work_dir}/.hivecode/permissions.local.yaml",
                    ],
        ...
        from hivecode.filehistory import FileHistory
        ...
        from hivecode.tools.exit_plan_mode import ExitPlanModeTool
        ...
        # --- Worktree 系统初始化 ---
        from hivecode.config import WorktreeConfig
        ...
        from hivecode.tools.enter_worktree import EnterWorktreeTool
        from hivecode.tools.exit_worktree import ExitWorktreeTool
        ...
        # --- Agent 团队系统初始化 ---
        from hivecode.teams.manager import TeamManager
        from hivecode.tools.team_create import TeamCreateTool
        from hivecode.tools.team_delete import TeamDeleteTool
        ...
        from hivecode.commands.handlers.trace import create_trace_command
        ...
        from hivecode.tools.synthetic_output import SyntheticOutputTool
    ...
    async def _prefetch_relevant_memories(self, query: str) -> str:
        ...
        async def selector(system_prompt: str, user_message: str) -> str:
            from hivecode.tools.base import StreamEnd, TextDelta
    ...
    async def _show_plan_approval(self) -> None:
        from hivecode.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from hivecode.plan_dialog import InlinePlanWidget, PlanChoice
        from hivecode.prompts import build_plan_mode_exit_reminder
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from hivecode.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from hivecode.permission_dialog import InlinePermissionWidget
