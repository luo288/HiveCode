...
from hivecode.teams.backend_detect import BackendDetectionError, detect_backend, detect_pane_backend
from hivecode.teams.coordinator import (
    get_coordinator_system_prompt,
    get_coordinator_user_context,
    is_coordinator_mode,
    match_session_mode,
)
from hivecode.agents.tool_filter import (
...
from hivecode.tools import ToolRegistry
from hivecode.tools.base import Tool, ToolResult
...
class TestModels:
    ...
    def test_unique_team_name(self, tmp_dir):
        with patch("hivecode.teams.models.Path.home", return_value=Path(tmp_dir)):
            name1 = unique_team_name("my-team")
            assert name1 == "my-team"
            (Path(tmp_dir) / ".hivecode" / "teams" / "my-team").mkdir(parents=True)
...
class TestBackendDetect:
    ...
    def test_pane_iterm2_with_it2(self):
        ...
        with patch.dict(os.environ, env, clear=False):
            with patch("hivecode.teams.backend_detect.shutil.which") as mock_which:
    ...
    def test_pane_no_backend_falls_back_to_in_process(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("TMUX", None)
            os.environ.pop("TERM_PROGRAM", None)
            with patch("hivecode.teams.backend_detect.shutil.which", return_value=None):
                result = detect_pane_backend()
                assert result == BackendType.IN_PROCESS
...
class TestCoordinatorMode:
    ...
    def test_enabled_with_flag_and_env(self):
        with patch.dict(os.environ, {"HIVECODE_COORDINATOR_MODE": "1"}):
            assert is_coordinator_mode(enable_flag=True) is True

    def test_flag_without_env(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HIVECODE_COORDINATOR_MODE", None)
    ...
    def test_match_session_mode_switch_to_coordinator(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            assert os.environ.get("HIVECODE_COORDINATOR_MODE") == "1"
...
class TestConfigExtensions:
    def test_teammate_mode_defaults(self):
        from hivecode.config import AppConfig
    ...
    def test_load_config_with_team_fields(self, tmp_dir):
        from hivecode.config import load_config
...
class TestTranscript:
    ...
    def test_save_and_load(self, tmp_dir):
        ...
        from hivecode.teams.transcript import load_transcript, save_transcript
    ...
    def test_load_nonexistent(self, tmp_dir):
        from hivecode.teams.transcript import load_transcript
        with patch("hivecode.teams.models.Path.home", return_value=Path(tmp_dir)):
...
class TestAgentCoordinatorIntegration:
    def test_normal_prompt(self):
        from hivecode.prompts import build_system_prompt, IDENTITY_SECTION
        prompt = build_system_prompt()
        # 验证 identity section 内容包含在 prompt 中
        assert "HiveCode" in prompt
        assert IDENTITY_SECTION.content[:30] in prompt

    def test_coordinator_prompt(self):
        from hivecode.prompts import build_system_prompt
